...
const Timer = () => {
  ...
  const checkForReminder = () => {
    if (state.seconds > 0 && state.seconds % (60 * reminderInterval) === 0) {
      ...
      msg.text =
        mins === 1
          ? `${state.seconds / 60} minute remains`
          : `${state.seconds / 60} minutes remain`;
  ...
  useEffect(() => {
    if (
      ...
      state.seconds <= warningThreshold &&
      state.seconds > 0
    ) {
      warningSound.play();
    }
  ...
  return (
    <div className="timer-page flex flex-col justify-center items-center bg-gradient-to-r from-cyan-500 to-blue-700 min-h-[calc(100vh-100px)]">
      <div className="timer-body flex flex-col my-10 h-160 w-10/12 bg-gray-700/90 rounded-xl max-w-[800px]">
        <h1 className="title text-[#B2DB1B] text-5xl mt-8 text-center">
          Timer
        </h1>
        ...
        <div onClick={allowTimeSetting} className="clock">
          <Display
            seconds={
              presetting
                ? clockInput.slice(4, 6).join("")
                : formatSeconds(state.seconds)
            }
            minutes={
              presetting
                ? clockInput.slice(2, 4).join("")
                : formatMinutes(state.seconds)
            }
            hours={
              presetting
                ? clockInput.slice(0, 2).join("")
                : formatHours(state.seconds)
            }
          />
        </div>
